...
void Tablero::dibujar() {
  ...
  for (int y = 0; y < dimension; y++) {
    ...
    for (int x = 0; x < dimension; ++x) {
      if (this->matriz[x][y] == Codigo::Agua) std::cout << std::left << std::setw(3) << std::setfill(' ') << '.';
      else if (this->matriz[x][y] == Codigo::Ataque) std::cout << std::left << std::setw(3) << std::setfill(' ') << '*';
      else if (this->matriz[x][y] == Codigo::Dañado) std::cout << std::left << std::setw(3) << std::setfill(' ') << 'X';
      else std::cout << std::left << std::setw(3) << std::setfill(' ') << this->matriz[x][y];
    }
    std::cout << "\n";
  }
}
