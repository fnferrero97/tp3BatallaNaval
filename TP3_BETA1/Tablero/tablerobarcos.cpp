...
bool TableroBarcos::sePuedeAgregar(Barco barco) {
  ...
  if (barco.getX() == Codigo::Ataque || barco.getY() == Codigo::Ataque)
    return false;
  ...
  if (barco.getOrientacion() == 'H') {
    ...
    finalJ = (barco.getX() + barco.getTamanio()) + 1 > dimension - 1
                 ? barco.getX() + barco.getTamanio()
                 : barco.getX() + barco.getTamanio() + 1;
  ...
  if (barco.getOrientacion() == 'V') {
    ...
    finalJ = (barco.getY() + barco.getTamanio()) + 1 > dimension - 1
                 ? barco.getY() + barco.getTamanio()
                 : barco.getY() + barco.getTamanio() + 1;
...
bool TableroBarcos::recibirAtaque(int x, int y) {
  ...
  if (this->matriz[x][y] == Codigo::Dañado)
    return false;
  ...
  for (int i = 0; i < cantBarcos; i++) {
    if (barcos[i]->getOrientacion() == 'H' && (y == barcos[i]->getY()) &&
        (x >= barcos[i]->getX() &&
         x < barcos[i]->getX() + barcos[i]->getTamanio())) {
      posEnCuerpo = (x - barcos[i]->getX());

      if (barcos[i]->golpe(posEnCuerpo))
        this->cantMuertos++;
    ...
    if (barcos[i]->getOrientacion() == 'V' && (x == barcos[i]->getX()) &&
        (y >= barcos[i]->getY() &&
         y < barcos[i]->getY() + barcos[i]->getTamanio())) {
...
bool TableroBarcos::gameOver() { return this->cantMuertos == maxBarcos; }

std::vector<Barco *> TableroBarcos::getBarcos() const { return this->barcos; }

int TableroBarcos::getCantBarcos() const { return cantBarcos; }
...
void TableroBarcos::moverLanchas() {
  if (this->tieneLancha) {
    ...
    for (int i = 0; i < maxBarcos; i++) {
      ...
      if (barcoAVerificar->getCodigo() == Codigo::Lancha &&
          !barcoAVerificar->isMuerto()) {
