...
void MainWindow::pintarMapa(bool band, QPushButton *button) {
  band ? button->setStyleSheet("background-color: green;")
       : button->setStyleSheet("background-color: blue;");
}

void MainWindow::describirAtaque(bool ataqueJugador, int x1, int y1,
                                 bool ataqueIA, int x2, int y2) {
  ataqueJugador ? ui->listaAtaquesJugador->setText(QString::fromStdString(
                      "Ataque EXITOSO en coordenadas x: " + std::to_string(y1) +
                      " / y: " + std::to_string(x1)))
                : ui->listaAtaquesJugador->setText(QString::fromStdString(
                      "Ataque FALLIDO en coordenadas x: " + std::to_string(y1) +
                      " / y: " + std::to_string(x1)));
  ataqueIA ? ui->listaAtaquesIA->setText(QString::fromStdString(
                 "Ataque EXITOSO en coordenadas x: " + std::to_string(y2) +
                 " / y: " + std::to_string(x2)))
           : ui->listaAtaquesIA->setText(QString::fromStdString(
                 "Ataque FALLIDO en coordenadas x: " + std::to_string(y2) +
                 " / y: " + std::to_string(x2)));
}
...
void MainWindow::crearMapas() {
  ...
  for (int i = 0; i < this->tamanioMapa; ++i) {
    ...
    for (int j = 0; j < this->tamanioMapa; ++j) {
      ...
      if (juego->getIA()->getTableroBarcos()->getMatriz()[i][j] ==
          Codigo::Dañado) {
        pintarMapa(true, buttons[i][j]);
      } else if (juego->getIA()->getTableroBarcos()->getMatriz()[i][j] ==
                 Codigo::Ataque) {
        pintarMapa(false, buttons[i][j]);
      } else {
      ...
      this->labels[i][j]->setStyleSheet(
          "outline: #87CEEB solid 5px; background-color: #87CEEB;");
...
void MainWindow::listarInfo() {
  ...
  for (auto barco :
       this->juego->getJugador()->getTableroBarcos()->getBarcos()) {
    ...
    if (!barco->isMuerto()) {
      ui->infoBarcos->append(
          QString::fromStdString(nombre + " (X: " + x + ", Y: " + y +
                                 ") - Vida: " + vida + "/" + tamanio));
    } else {
      ui->infoBarcos->append(QString::fromStdString(
          nombre + " (X: " + x + ", Y: " + y + ") - HUNDIDO!"));
      if ((barco->getCodigo() == Codigo::Submarino) && !barco->isHundido())
        eliminarSubmarino(barco, this->juego->getJugador());
    }
  }
  ...
  for (auto barco : this->juego->getIA()->getTableroBarcos()->getBarcos()) {
    ...
    if (!barco->isMuerto()) {
      ...
      if ((barco->getCodigo() == Codigo::Submarino) && !barco->isHundido())
        eliminarSubmarino(barco, this->juego->getIA());
    }
  }
}
...
void MainWindow::goMenuPrincipal() {
  this->ui->nuevoJuego->show();
  this->ui->cargarPartida->show();
}

void MainWindow::resetearParametros() {
  if (this->juego) {
    delete this->juego;
    this->juego = nullptr;
  };
  this->tamanioMapa = 0;
  ui->cantBarcos->setText("");
  ui->tamanioMapa->setText("");
  ui->checkAleatorio->setChecked(false);
  ...
  for (int i = 0; i < this->tamanioMapa; i++) {
    for (int j = 0; j < this->tamanioMapa; j++) {
      if (matriz[i][j] == Codigo::Agua) {
        QPixmap imgLabel =
            QPixmap(":/images/agua.png").scaled(50, 50, Qt::KeepAspectRatio);
        this->labels[i][j]->setPixmap(imgLabel);
      } else if (matriz[i][j] == Codigo::Ataque) {
        QPixmap imgLabel =
            QPixmap(":/images/errado.png").scaled(50, 50, Qt::KeepAspectRatio);
        this->labels[i][j]->setPixmap(imgLabel);
      } else if (matriz[i][j] == Codigo::Dañado) {
        QPixmap imgLabel =
            QPixmap(":/images/impacto.png").scaled(50, 50, Qt::KeepAspectRatio);
        this->labels[i][j]->setPixmap(imgLabel);
      } else
        this->labels[i][j]->setText(
            QString::fromStdString(std::to_string(matriz[i][j])));
    }
  }
}
...
void MainWindow::on_aniadirBarco_clicked() {
  ...
  if ((!ui->posX->text().isEmpty() && !ui->posY->text().isEmpty()) &&
      this->juego->colocarBarcosManual(opcionBarco, x, y, orientacion) &&
      ui->listaBarcos->currentIndex() != -1) {
...
void MainWindow::setContadorBarcos(int valor, int total) {
  ui->barcosRestantes->setText(
      QString::fromStdString("Barcos agregados: " + std::to_string(valor) +
                             " / " + std::to_string(total)));
}
...
void MainWindow::on_cargarPartida_clicked() {
  ...
  for (int i = 0; i < cantBarcos; ++i) {
    archi.read((char *)&barcoStr, sizeof(BarcoStr));
    // auto [codigo, x, y, orientacion] = barcoStr;
    juego->colocarBarcos(this->juego->getIA(), barcoStr.codigo, barcoStr.x,
                         barcoStr.y, barcoStr.orientacion);
  }
  ...
  while (archi.read((char *)&ataqueStr, sizeof(AtaqueStr))) {
    if (atacarIA) {
      juego->atacarIA(ataqueStr.x, ataqueStr.y);
    } else {
...
void MainWindow::on_listaBarcos_currentIndexChanged(int index) {
  ...
  case 0: {
    ...
    ui->nombreBarco->setText(
        QString::fromStdString("Crucero - 3 Puntos de vida"));
  ...
  case 1: {
    ...
    ui->nombreBarco->setText(
        QString::fromStdString("Destructor - 4 Puntos de vida"));
  ...
  case 2: {
    ...
    ui->nombreBarco->setText(
        QString::fromStdString("Fragata - 2 Puntos de vida"));
    ui->imagenBarco->setPixmap(imgBarco.scaled(200, 200, Qt::KeepAspectRatio));
    break;
  }
  case 3: {
    QPixmap imgBarco(":/images/lancha.png");
    ui->nombreBarco->setText(QString::fromStdString(
        "Lancha - 1 Punto de vida - Si no es alcanzada entonces se re "
        "posiciona aleatoreamiente en algún lugar disponible."));
  ...
  case 4: {
    ...
    ui->nombreBarco->setText(
        QString::fromStdString("Portaavion - 5 Puntos de Vida"));
  ...
  case 5: {
    ...
    ui->nombreBarco->setText(
        QString::fromStdString("Submarino - 3 Puntos de Vida - Es eliminado "
                               "con un golpe a su centro."));
    ui->imagenBarco->setPixmap(imgBarco.scaled(200, 200, Qt::KeepAspectRatio));
    break;
  }
  }
...
void MainWindow::on_guardarPartida_clicked() {
  /*
  FORMATO GUARDADO:
  Tamaño mapa: m
  Cant barcos: n
  {cod_barco, x, y, orientacion} ... n veces para jugador
  {cod_barco, x, y, orientacion} ... n veces para IA
  {x,y} ... ataques, intercalados entre el Jugador y la IA
  */
  ...
  for (auto barco :
       this->juego->getJugador()->getTableroBarcos()->getBarcos()) {
    BarcoStr barcoStr = barco->getStruct();
    archi.write((char *)&barcoStr, sizeof(BarcoStr));
  }
  for (auto barco : this->juego->getIA()->getTableroBarcos()->getBarcos()) {
    BarcoStr barcoStr = barco->getStruct();
    archi.write((char *)&barcoStr, sizeof(BarcoStr));
  ...
  for (std::pair<int, int> ataque : this->juego->getAtaques()) {
    AtaqueStr ataqueStr{ataque.first, ataque.second};
    archi.write((char *)&ataqueStr, sizeof(AtaqueStr));
  }
  archi.close();
