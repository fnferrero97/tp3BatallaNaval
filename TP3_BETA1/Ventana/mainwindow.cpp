...
void MainWindow::describirAtaque(bool ataqueJugador, int x1, int y1,
  ...
  ataqueJugador ? ui->listaAtaquesJugador->setText(QString::fromStdString(
                      "Ataque EXITOSO en coordenadas x: " + std::to_string(x1) +
                      " / y: " + std::to_string(y1)))
                : ui->listaAtaquesJugador->setText(QString::fromStdString(
                      "Ataque FALLIDO en coordenadas x: " + std::to_string(x1) +
                      " / y: " + std::to_string(y1)));
  ataqueIA ? ui->listaAtaquesIA->setText(QString::fromStdString(
                 "Ataque EXITOSO en coordenadas x: " + std::to_string(x2) +
                 " / y: " + std::to_string(y2)))
           : ui->listaAtaquesIA->setText(QString::fromStdString(
                 "Ataque FALLIDO en coordenadas x: " + std::to_string(x2) +
                 " / y: " + std::to_string(y2)));
}
...
void MainWindow::listarInfo() {
  ...
  for (auto barco : this->juego->getIA()->getTableroBarcos()->getBarcos()) {
    //    std::string nombre(barco->getNombre());
    //    if (!barco->isMuerto()) {
    //      ui->infoBarcos->append(QString::fromStdString(nombre + " - A
    //      FLOTE!"));
    //    } else {
    //      ui->infoBarcos->append(QString::fromStdString(nombre + " -
    //      HUNDIDO!"));
    //    }
    std::string x = std::to_string(barco->getX());
    std::string y = std::to_string(barco->getY());
  ...
  for (int i = 0; i < this->tamanioMapa; i++) {
    for (int j = 0; j < this->tamanioMapa; j++) {
      if (matriz[i][j] == Codigo::Agua)
        this->labels[i][j]->setText(QString::fromStdString("--"));
      else if (matriz[i][j] == Codigo::Ataque)
        this->labels[i][j]->setText(QString::fromStdString("**"));
      else if (matriz[i][j] == Codigo::Dañado)
        this->labels[i][j]->setText(QString::fromStdString("XX"));
      else
